...
def write_python_sqlite(target: Path, parent_sha256: str) -> None:
    text = f'''"""Generated ASET 0.4alpha Python SQLite persistence extension.

Authority-Seeded Evidence Trail (ASET). Semantic delta: none.
"""
...
def _load_parent() -> Any:
    if _sha256(PARENT_PATH) != PARENT_SHA256:
        raise RuntimeError("parent Python expression byte identity mismatch")
    spec = importlib.util.spec_from_file_location(
        "aset_seed_alpha4_parent",
        PARENT_PATH,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("parent Python expression cannot be loaded")
    ...
    def apply_component(
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            result = _parent.apply_component(
                current,
                component_id,
                evidence=evidence,
...
def write_python_sqlite_binding(
    ...
    value = {
        ...
        "assurance_scope": [
            "EXACT_PARENT_BYTE_BINDING",
            "PARENT_OBSERVATIONAL_CONGRUENCE",
            "ATOMIC_SUCCESSFUL_COMMIT",
            "ROLLBACK_ON_PARENT_REJECTION",
            "RESTART_ROUND_TRIP",
        ],
        "not_claimed": [
...
def build_release_profiles(root: Path, output: Path) -> None:
    ...
    parent = output / "python/aset_seed_alpha4.py"
    extension = output / "python-sqlite/aset_seed_alpha4_sqlite.py"
    binding = output / "python-sqlite/PERSISTENCE_EXTENSION.json"
    write_python(parent, records)
    write_python_sqlite(extension, sha256(parent))
    write_python_sqlite_binding(binding, parent, extension)
