...
Defines closed schemas, canonicalization, signatures, and portable wire contracts.
...
Validate a normative artifact with its exact closed-world schema.
...
Produce ASET-JCS-1 canonical bytes and digest without business decisions.
...
- `INV-SCHEMA-003` — The externally signed freeze profile binds the path, schema $id, byte SHA-256 and canonical digest of every mandatory normative JSON Schema before any schema is used.
