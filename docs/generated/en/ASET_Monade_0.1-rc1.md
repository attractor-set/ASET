...
Coordinates Task, execution, and independent result acceptance above Seed-authorized transitions.
...
Admit a scoped Memory projection into Task context.
...
Bind authorized expected change to one operational execution.
...
Dispatch one externally effective execution through an outbox boundary.
...
Admit an observation bound to the exact execution attempt.
...
Evaluate evidence independently from the primary executor.
...
Recognize the terminal task Outcome only from complete PASS verification semantics.
...
Retry only under NO_EFFECT or proved idempotency.
...
Perform a separately authorized compensation effect.
