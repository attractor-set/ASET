...
Координирует Task, исполнение и независимое принятие результата поверх разрешённых переходов Seed.
...
Допускает проекцию контекста в Task без превращения найденного материала в проверенный факт.
...
Связывает разрешённое ожидаемое изменение с точным исполнителем, инструментом и критериями.
...
Материализует ExecutionIntent и передаёт внешний эффект только после отдельного execution Permit.
...
Регистрирует наблюдение внешнего исполнения, не объявляя его проверенным результатом.
...
Проверяет evidence относительно AcceptanceContract независимо от исполнителя.
...
Признаёт Outcome только на основании допустимой Verification и явной классификации результата.
...
Создаёт новую разрешённую попытку, не повторно используя ранее потреблённый Permit.
...
Выполняет отдельно разрешённое компенсирующее действие с собственной цепочкой evidence.
