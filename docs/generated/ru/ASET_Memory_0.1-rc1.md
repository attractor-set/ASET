...
Сохраняет происхождение, эпизоды и проекции памяти без превращения retrieval в полномочие или проверенный факт.
...
Строит ограниченную проекцию памяти с происхождением и явным эпистемическим статусом.
...
Изменяет управляемую память только через разрешённый атомарный переход.
