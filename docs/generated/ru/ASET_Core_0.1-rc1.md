...
Определяет нормативное разрешение, одноразовый Permit и атомарное пересечение gate.
...
Формирует нормативное решение для точного документа и заданного gate.
...
Выдаёт одноразовый Permit только после разрешающего нормативного решения.
...
Атомарно потребляет Permit, применяет patch и создаёт receipt пересечения.
