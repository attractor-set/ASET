...
Определяет контекст как версионируемый namespace, его неизменяемые компоненты, проекции, композицию и атомарные patch-переходы.
...
Вычисляет детерминированный составной корень из неизменяемых подписанных компонентов.
...
Создаёт новую версию контекста посредством разрешённого атомарного набора изменений.
