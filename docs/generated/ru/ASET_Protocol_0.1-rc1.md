...
Определяет закрытые схемы, канонизацию, подписи и переносимые wire-контракты.
...
Проверяет нормативный документ по закрытой схеме до его допуска.
...
Создаёт канонические байты и digest без выполнения бизнес-логики.
